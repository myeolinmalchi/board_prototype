...
trait Tables {
  ...
  case class BoardsRow(boardId: Int, name: String = "")
  /** GetResult implicit for fetching BoardsRow objects using plain SQL queries */
  implicit def GetResultBoardsRow(implicit e0: GR[Int], e1: GR[String]): GR[BoardsRow] = GR{
    prs => import prs._
    BoardsRow.tupled((<<[Int], <<[String]))
  }
